...
module.exports = {
	...
	run: async ({ client, message, args }) => {
		...
		if (!member) return message.reply({ content: `Bir kullanıcı belirt. \`.k [@Shainey/ID] Aslı 17\`` }).then(message.react(config.Others.Emojis.reject)).then(message.react(config.Others.Emojis.reject)).sil(3);

		const name = args.slice(1).filter(arg => isNaN(arg)).map(arg => arg[0].toUpperCase() + arg.slice(1).toLowerCase()).join(" ");
		if (!name) return message.reply({ content: `Bir isim belirt. \`.k [@Shainey/ID] Aslı 17\`` }).then(message.react(config.Others.Emojis.reject)).then(message.react(config.Others.Emojis.reject)).sil(3);
		...
		Schema.findOne({ guildID: message.guild.id, userID: member.id }, async (err, shi) => {
			if (!shi) {
				let seul = [];
				seul.push({ Names: Nickname, Sex: `Kadın`, Staff: message.author.id, Date: Date.now() })
				...
				shi.Names.push({ Names: Nickname, Sex: `Kadın`, Staff: message.author.id, Date: Date.now() })
		...
		setTimeout(() => { member.roles.add(config.Roles.Member.womanRoles).catch((err) => console.log(`${err}`)) }, 1000)
